...
from app.pipeline.seed import seed_initial_data
...
def download_and_ingest_historical_csvs(db: Session, competition: Competition):
    ...
    for season in seasons:
        ...
        try:
            ...
            existing_assocs = db.query(TeamSeason).filter(TeamSeason.season_id == season.id).all()
...
def precompute_standings(db: Session, competition: Competition):
    ...
    db.query(GameweekStanding).filter(GameweekStanding.competition_id == competition.id).delete()
    db.commit()
    ...
    for season in seasons:
        ...
        team_seasons = db.query(TeamSeason).filter(TeamSeason.season_id == season.id).all()
        ...
        max_games_played = max([len(m_list) for m_list in teams_matches_played.values()]) if teams_matches_played else 0
        
        for round_num in range(1, max_games_played + 1):
            round_teams_data = []
            
            for team_id, match_list in teams_matches_played.items():
                # We calculate the statistics up to the round_num-th match played by this team
                # If a team has played fewer matches than round_num, they don't have a record for this round yet
            ...
            def get_ranking_key(team_data):
                ...
                return (-team_data["points"], -team_data["goal_difference"], -team_data["goals_for"], team_name)
...
def sync_fpl_players_and_summaries(db: Session, competition: Competition):
    ...
    try:
        ...
        for fp in fpl_players:
            ...
            player = db.query(Player).filter(Player.current_fpl_id == current_fpl_id).first()
            if not player:
        ...
        for db_id, fpl_id, full_name in active_player_ids:
            ...
            summary_url = f"https://fantasy.premierleague.com/api/element-summary/{fpl_id}/"
            ...
            try:
                ...
                for hp in history_past:
                    ...
                    season_record = db.query(Season).filter(Season.label == normalized_label).first()
                    if not season_record:
                        continue
                        
                    # Save summary
                    existing_summary = db.query(PlayerSeasonSummary).filter(
                ...
                current_season_record = db.query(Season).order_by(Season.id.desc()).first() # Latest season
                if current_season_record:
                    history = p_data.get("history", [])
                    for h in history:
                        opponent_fpl_id = h["opponent_team"]
                        kickoff_str = h.get("kickoff_time") # e.g. "2026-05-18T19:00:00Z"
                        
                        if kickoff_str:
                            try:
                                kickoff_date = datetime.strptime(kickoff_str[:10], "%Y-%m-%d").date()
                            except ValueError:
                                continue
                            ...
                            if opponent_db_team:
                                ...
                                match_record = db.query(Match).filter(
                                    ...
                                    or_(Match.home_team_id == opponent_db_team.id, Match.away_team_id == opponent_db_team.id)
                                ).first()
                                
                                if match_record:
                                    existing_match_stat = db.query(PlayerMatchStat).filter(
                                    ...
                                    if not existing_match_stat:
                                        ...
                                        existing_match_stat.minutes = h.get("minutes", 0)
                                        existing_match_stat.goals = h.get("goals_scored", 0)
                                        existing_match_stat.assists = h.get("assists", 0)
                                        existing_match_stat.clean_sheets = h.get("clean_sheets", 0)
                                        existing_match_stat.yellow_cards = h.get("yellow_cards", 0)
                                        existing_match_stat.red_cards = h.get("red_cards", 0)
                                        existing_match_stat.fpl_points = h.get("total_points", 0)
