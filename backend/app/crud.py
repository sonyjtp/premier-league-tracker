...
from app.models import (
    Competition, Season, Team, TeamSeason, Match, GameweekStanding,
    Player, PlayerSeasonSummary, PlayerMatchStat,
...
def get_standings(db: Session, season_id: int, gameweek: Optional[int] = None,
                  competition_code: str = "EPL") -> List[GameweekStanding]:
    comp = db.query(Competition).filter(Competition.code == competition_code).first()
    if not comp:
        return []
    ...
    if not gameweek:
        return []
    return (db.query(GameweekStanding)
            .filter(GameweekStanding.season_id == season_id,
...
def get_head_to_head(db: Session, team_a_id: int, team_b_id: int, limit: int = 10) -> List[Match]:
...
def upsert_player_profile(db: Session, player_id: int, data: Dict[str, Any]) -> None:
    ...
    if profile:
        ...
        profile = PlayerProfile(player_id=player_id, updated_at=datetime.utcnow(), **data)
        db.add(profile)
    db.commit()
...
def store_match_events(db: Session, fixture_api_id: int, events: List[Dict]) -> None:
    for ev in events:
        exists = db.query(MatchEvent).filter(
...
def get_match_lineups(db: Session, fixture_api_id: int) -> List[MatchLineup]:
    return db.query(MatchLineup).filter(MatchLineup.fixture_api_id == fixture_api_id).all()

def store_match_lineups(db: Session, fixture_api_id: int, entries: List[Dict]) -> None:
    for entry in entries:
        exists = db.query(MatchLineup).filter(
...
def get_match_advanced_stats(db: Session, match_id: int) -> Optional[MatchAdvancedStats]:
    return db.query(MatchAdvancedStats).filter(MatchAdvancedStats.match_id == match_id).first()

def upsert_match_advanced_stats(db: Session, match_id: int, data: Dict[str, Any]) -> None:
    existing = db.query(MatchAdvancedStats).filter(MatchAdvancedStats.match_id == match_id).first()
    ...
    if existing:
        ...
        db.add(MatchAdvancedStats(match_id=match_id, fetched_at=datetime.utcnow(), **fields))
    db.commit()

def get_player_advanced_stats(db: Session, player_id: int) -> List[PlayerAdvancedStats]:
    return (db.query(PlayerAdvancedStats)
            .filter(PlayerAdvancedStats.player_id == player_id)
            .join(Season)
            .order_by(Season.id).all())

def upsert_player_advanced_stats(db: Session, player_id: int, season_id: int,
                                  data: Dict[str, Any]) -> None:
    existing = db.query(PlayerAdvancedStats).filter(
        PlayerAdvancedStats.player_id == player_id,
        PlayerAdvancedStats.season_id == season_id,
    ).first()
    fields = {k: data.get(k) for k in (
        "xg", "xa", "npxg", "xg_per_90", "xa_per_90",
    ...
    if existing:
        ...
        db.add(PlayerAdvancedStats(
            player_id=player_id, season_id=season_id,
            fetched_at=datetime.utcnow(), **fields,
        ))
    db.commit()
